...
$pageTitle = ucfirst($preferred) . ' sitter';

function roomHasFreeBed(array $room): bool
{
    if (array_key_exists('has_free_bed', $room)) {
        return (bool) $room['has_free_bed'];
    }

    if (($room['type'] ?? '') === 'single') {
        return empty($room['student1_id']);
    }

    return empty($room['student1_id']) || empty($room['student2_id']);
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <link rel="stylesheet" href="<?= BASE_URL ?>public/css/student.css?v=2">
</head>
<body class="rs-body">
    ...
    <div class="rs-container">
        ...
        <div class="rs-left">
            ...
            <div class="rs-room-list" id="roomList">
                <?php if (empty($rooms)): ?>
                    <p class="rs-no-rooms">No <?= htmlspecialchars($preferred) ?> rooms found.<br>Please contact the warden.</p>
                <?php else: ?>
                    <?php foreach ($rooms as $i => $r): ?>
                        <?php $hasFreeBed = roomHasFreeBed($r); ?>
                        <button
                            class="rs-room-pill <?= $i === 0 ? 'active' : '' ?> <?= $hasFreeBed ? 'is-available' : 'is-full' ?>"
                            data-room-id="<?= $r['id'] ?>"
                            ...
                            data-has-free-bed="<?= $hasFreeBed ? '1' : '0' ?>"
                        >
        ...
        <div class="rs-right">
            ...
            <?php else: ?>
                <div class="rs-detail rs-empty-state">
                    <i class="ph ph-bed"></i>
                    <p>No rooms found for your room type.<br>Contact the warden to be assigned manually.</p>
...
function bedSVG(occupied) {
    const color = occupied ? '#ff1f1f' : '#18a558';
    return `<svg viewBox="0 0 120 70" xmlns="http://www.w3.org/2000/svg" class="rs-bed-svg">
        <path d="M12 16v46M108 34v28M12 34h64c11 0 20 9 20 20v8" fill="none" stroke="${color}" stroke-width="6" stroke-linecap="round" stroke-linejoin="round"/>
        <path d="M12 34h36v28M48 34h48" fill="none" stroke="${color}" stroke-width="5" stroke-linecap="round" stroke-linejoin="round"/>
    </svg>`;
}
...
function bedBlock(slot, occupied, selected, occupantName) {
    const detailText = occupied
        ? (occupantName ? escapeHtml(occupantName) : 'Taken')
        : (selected ? 'Selected' : 'Available');

    return `<button
        type="button"
        class="rs-bed-block ${occupied ? 'is-occupied' : 'is-empty'} ${selected ? 'selected' : ''}"
        data-slot="${slot}"
    ...
    >
        ...
        ${bedSVG(occupied)}
        <span class="rs-bed-choice">${detailText}</span>
    </button>`;
}
...
function renderBeds(room) {
    ...
    if (room.type === 'single') {
        inputBedSlot.value = s1 ? '' : 'student1';
        bedsWrap.innerHTML = `
            ${bedBlock('student1', s1, !s1, room.student1_name)}`;
    } else {
        ...
        bedsWrap.innerHTML = `
            ${bedBlock('student1', s1, inputBedSlot.value === 'student1', room.student1_name)}
            ${bedBlock('student2', s2, inputBedSlot.value === 'student2', room.student2_name)}`;
    }
...
bedsWrap && bedsWrap.addEventListener('click', function(e) {
    const bed = e.target.closest('.rs-bed-block');
    if (!bed || bed.dataset.occupied === '1') return;

    inputBedSlot.value = bed.dataset.slot;
    bedsWrap.querySelectorAll('.rs-bed-block').forEach(block => block.classList.remove('selected'));
